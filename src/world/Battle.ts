...
export default class Battle {
	...
	private enemyPokemonSprites: HTMLImageElement[] = [];
	...
	private eventQueue: SocketBattleTurnEvent[] = [];
	...
	constructor(world: World, player: Player, enemy: Player) {
		this.world = world;
		this.player = player;
		this.enemy = enemy;
		const firstIndexWhereHealthyPokemon = this.player.party.findIndex(pokemon => pokemon?.hp > 0);
		const firstIndexWhereHealthyEnemyPokemon = this.enemy.party.findIndex(pokemon => pokemon?.hp > 0);
		this.playerCurrentPokemonIndex = firstIndexWhereHealthyPokemon;
		this.enemyCurrentPokemonIndex = firstIndexWhereHealthyEnemyPokemon;

		this.loadAssets();

		this.playerPokemonHp = this.player.party.filter(pokemon => pokemon !== null)[this.playerCurrentPokemonIndex].hp;
		this.enemyPokemonHp = this.enemy.party.filter(pokemon => pokemon !== null)[this.enemyCurrentPokemonIndex].hp;
	}

	public initBattle() {
		this.world.client.audioManager.isAutoplayOn = false;
		this.world.client.audioManager.playMusic('battle_rival.ogg');
	}
	...
	private processBattleEvents(): void {
		const battleTurnEvents = this.world.battleTurnEvents;

		if (battleTurnEvents.length > 0) {
			this.eventQueue = [...battleTurnEvents];
		}
					...
					const interval = setInterval(() => {
						if (this.enemyPokemonHp > pokemonHpAfterMove) {
							...
							if (event.isAllPokemonsFainted) {
								setTimeout(() => {
									this.displayedTextRow1 = `Enemy ${this.enemy.name} was defeated!`;
									this.displayedTextRow2 = 'Click here to continue...';
									this.world.client.audioManager.playMusic('victory.ogg');
								}, 1000);
							} else if (event.isBattleOver) {
	...
	private setCurrentPokemon(event: SocketBattleTurnEvent) {
		const isCurrentPlayer = event.playerID === this.player.entityID;
		const pokemonIndex = event.switchedPokemonIndex;
		if (isCurrentPlayer) {
			this.playerCurrentPokemonIndex = pokemonIndex;
			this.playerPokemonHp = this.player.party.filter(pokemon => pokemon !== null)[
			...
			console.log('Enemy current pokemon index:', this.enemyCurrentPokemonIndex);
			this.enemyPokemonHp = this.enemy.party.filter(pokemon => pokemon !== null)[pokemonIndex].hp;
			console.log('Enemy current pokemon hp:', this.enemyPokemonHp);
		}
	...
	private handleRun(event: SocketBattleTurnEvent): void {
	...
	private handleItemUse(event: SocketBattleTurnEvent): void {
	...
	public draw() {
		...
		if (this.isWaitingForOtherPlayer) {
			this.displayedTextRow1 = 'Waiting for the other player...';
		}
		...
		if (this.isThrowingPokeball) {
			...
			if (this.pokeballThrowCounter < 45) {
				...
				if (!this.pokeballTopHalfImage || !this.pokeballBottomHalfImage) return;
	...
	private drawIntro(): void {
		...
		if (this.currentBattleIntroFrame * 5 === 750) {
			...
			this.displayedTextRow1 = `Enemy ${this.enemy.name} wants to battle!`;
	...
	public handleClick(event: MouseEvent) {
		...
		if (this.activeOption === null) {
			...
			if (this.isMouseOverRun) {
				this.displayedTextRow1 = "No! There's no running away";
				this.displayedTextRow2 = 'from a trainer battle!';
				setTimeout(() => {
					this.displayedTextRow1 = '';
					this.displayedTextRow2 = '';
				}, 3000);
		...
		if (this.activeOption === 'fight') {
			...
			if (this.hoveredItem !== null) {
				...
				this.activeOption = null;
				this.displayedTextRow1 = "Can't use that here!";
				setTimeout(() => {
					this.displayedTextRow1 = '';
				}, 3000);
				return;
